...
function App() {
  ...
  const displayName = useMemo(() => {
    return currentUser?.fullName || currentUser?.name || currentUser?.username || 'there';
  }, [currentUser]);
  ...
  const copyToClipboard = async (value, label) => {
    try {
      ...
      // Fallback: best-effort prompt for older browsers/permissions
  ...
  const quickActions = useMemo(() => {
    ...
    if (role === 'MANAGER') {
      return [
        { key: 'dashboard', label: 'Open dashboard', icon: '📊', onClick: () => setCurrentPage('dashboard') },
        ...common,
      ];
    }
    ...
    if (role === 'STAFF') {
      return [
        ...common,
        { key: 'dashboard', label: 'Usage overview', icon: '📈', onClick: () => setCurrentPage('dashboard') },
      ];
    }
